  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-indigo-900">
      <div className="max-w-7xl mx-auto px-4 py-4 sm:py-6">
        {/* Header - Responsive with icons only on mobile */}
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-3">
          <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent flex items-center gap-2">
            <span>🎮</span> Admin
          </h1>
          <div className="flex gap-2">
            <button 
              onClick={() => setShowGameEditor(!showGameEditor)} 
              className="bg-yellow-600 hover:bg-yellow-700 text-white p-2 sm:px-4 sm:py-2 rounded-full transition-all font-semibold shadow-lg flex items-center gap-1"
              title="Edit Game"
            >
              <span>✏️</span>
            ...
            <button 
              ...
              className="bg-green-600 hover:bg-green-700 text-white p-2 sm:px-4 sm:py-2 rounded-full transition-all font-semibold shadow-lg flex items-center gap-1"
              title="New Game"
            >
              <span>🎮</span>
            ...
            <button 
              ...
              className="bg-gray-700 hover:bg-gray-600 text-white p-2 sm:px-4 sm:py-2 rounded-full transition-all flex items-center gap-1"
              title="Back"
            >
              <span>←</span>
        ...
        {/* Active Game Banner - Responsive */}
        {!hasActiveGame && (
          <div className="bg-red-500/20 border border-red-500 rounded-2xl p-3 sm:p-4 mb-6 backdrop-blur-sm">
            <div className="flex justify-between items-center flex-wrap gap-3">
              <div className="flex items-center gap-2 sm:gap-3">
                <span className="text-2xl sm:text-4xl">⚠️</span>
                <div>
                  <div className="text-white font-bold text-sm sm:text-lg">No Active Game Round!</div>
                  <div className="text-red-200 text-xs sm:text-sm">Create a new game to start the round</div>
                </div>
              </div>
              <button onClick={() => setShowGameCreator(true)} className="bg-green-600 text-white px-4 sm:px-6 py-1 sm:py-2 rounded-full text-sm sm:text-base font-bold">
                🎮 Create Game
              </button>
            </div>
          </div>
        ...
        {activeGame && hasActiveGame && (
          <div className="bg-gradient-to-r from-purple-600 to-indigo-700 rounded-2xl p-4 sm:p-5 mb-6 shadow-xl overflow-x-auto">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 text-white min-w-[500px] sm:min-w-0">
              <div>
                <div className="text-xs opacity-80">Game</div>
                <div className="text-lg sm:text-2xl font-bold">#{activeGame.round || activeGame.game_id}</div>
              </div>
              <div>
                <div className="text-xs opacity-80">Price</div>
                <div className="text-base sm:text-xl font-bold">{activeGame.price_per_number} ETB</div>
              </div>
              <div>
                <div className="text-xs opacity-80">Range</div>
                <div className="text-sm sm:text-lg font-bold">{activeGame.min_number} - {activeGame.max_number}</div>
              </div>
              <div>
                <div className="text-xs opacity-80">Total</div>
                <div className="text-base sm:text-xl font-bold">{activeGame.total_numbers}</div>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {[
            ...
            <motion.div
              ...
              className={`bg-gradient-to-r ${stat.color} rounded-xl p-3 sm:p-4 text-white shadow-lg`}
            >
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {[
            ...
            <button
              ...
              className={`px-3 sm:px-5 py-1.5 sm:py-2 rounded-full text-sm sm:text-base font-semibold transition-all ${
                activeTab === tab.id
        ...
        <AnimatePresence>
          {showGameEditor && activeGame && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowGameEditor(false)}>
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-gray-900 rounded-2xl p-5 sm:p-6 max-w-md w-full border border-purple-500 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <h2 className="text-xl sm:text-2xl font-bold text-white mb-4">✏️ Edit Game #{activeGame.game_id}</h2>
                <div className="space-y-4">
                  <div>
                    <label className="text-gray-300 block mb-2 text-sm">Price (ETB):</label>
                  ...
                  <div>
                    <label className="text-gray-300 block mb-2 text-sm">Number Range:</label>
                    <div className="flex gap-2">
                  ...
                  <div>
                    <label className="text-gray-300 block mb-2 text-sm">Status:</label>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={updateCurrentGame} className="flex-1 bg-yellow-600 text-white py-2 rounded-full font-bold text-sm sm:text-base">💾 Save</button>
                  <button onClick={() => setShowGameEditor(false)} className="flex-1 bg-gray-700 text-white py-2 rounded-full font-bold text-sm sm:text-base">Cancel</button>
                </div>
              </motion.div>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {showGameCreator && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowGameCreator(false)}>
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-gray-900 rounded-2xl p-5 sm:p-6 max-w-md w-full border border-green-500 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <h2 className="text-xl sm:text-2xl font-bold text-white mb-4">🎮 Create New Game</h2>
                <div className="space-y-4">
                  <div>
                    <label className="text-gray-300 block mb-2 text-sm">Price (ETB):</label>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={createNewGame} className="flex-1 bg-green-600 text-white py-2 rounded-full font-bold text-sm sm:text-base">✅ Create</button>
                  <button onClick={() => setShowGameCreator(false)} className="flex-1 bg-gray-700 text-white py-2 rounded-full font-bold text-sm sm:text-base">Cancel</button>
                </div>
              </motion.div>
            </motion.div>
          )}
        ...
        {/* Rest of the tabs remain the same but with responsive padding */}
        {activeTab === "pending" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6">
                  ...
                  <motion.div key={payment.payment_id} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: idx * 0.05 }} className="bg-gray-700/50 rounded-xl p-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
                      <div className="flex-1 w-full sm:w-auto">
                        ...
                        <div className="text-base sm:text-lg text-purple-400 font-bold mt-1">Number: {payment.number} | {payment.amount} ETB</div>
                        <div className="text-xs text-gray-500 break-all">Payment ID: {payment.payment_id}</div>
                      </div>
                      {payment.image_url && (
                        <img src={payment.image_url} alt="Payment proof" className="w-16 h-16 rounded-lg object-cover border border-gray-600 cursor-pointer hover:opacity-80 flex-shrink-0" onClick={() => window.open(payment.image_url, "_blank")} />
                      )}
                      <div className="flex gap-2 w-full sm:w-auto">
                        <button onClick={() => approvePayment(payment.payment_id)} className="bg-green-600 hover:bg-green-700 text-white px-3 sm:px-4 py-2 rounded-full text-sm transition-all flex-1 sm:flex-none">✅ Approve</button>
                        <button onClick={() => rejectPayment(payment.payment_id)} className="bg-red-600 hover:bg-red-700 text-white px-3 sm:px-4 py-2 rounded-full text-sm transition-all flex-1 sm:flex-none">❌ Reject</button>
                      </div>
                    </div>
                  </motion.div>
                ))}
        ...
        {/* All Users Tab - Responsive */}
        {activeTab === "users" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-4">👥 All Users</h2>
                ...
                <div key={user.telegram_id} className="bg-gray-700/50 rounded-lg p-3">
                  <div className="flex flex-col sm:flex-row justify-between gap-2">
                    <div className="break-all">
                      ...
                      <div className="text-xs text-green-400 break-all">✅ Approved: {user.selected_numbers?.join(', ') || 'None'}</div>
        ...
        {activeTab === "winner" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-4">🏆 Select Winners</h2>
            <div className="mb-4">
              <label className="text-gray-300 block mb-2 text-sm">Number of Winners:</label>
            ...
            <div className="mb-6">
              <label className="text-gray-300 block mb-2 text-sm">Prize Amounts (ETB):</label>
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {[...Array(numPlaces)].map((_, index) => (
                  <div key={index} className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-center">
                    <div className="w-full sm:w-32 text-gray-400 text-sm">Place #{index + 1}:</div>
                    <input type="number" value={prizeAmounts[index + 1] || ""} onChange={(e) => setPrizeAmounts({ ...prizeAmounts, [index + 1]: parseInt(e.target.value) })} className="flex-1 w-full bg-gray-700 text-white px-4 py-2 rounded-xl border border-gray-600" />
                    <span className="text-gray-400 text-sm">ETB</span>
                  </div>
                ))}
              </div>
            </div>
            <button onClick={selectMultiWinners} className="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white py-3 rounded-full font-bold text-base sm:text-lg hover:scale-105 transition-all">
              🎲 SELECT {numPlaces} WINNER{numPlaces > 1 ? 'S' : ''}
            </button>
            {winnerResult && (
              <div className="mt-6">
                <h3 className="text-white font-bold text-base sm:text-lg mb-3">🎉 WINNERS SELECTED!</h3>
                    ...
                    <div key={winner.place} className={`bg-gradient-to-r ${getPlaceBadge(winner.place)} rounded-xl p-4 text-white`}>
                      <div className="flex flex-col sm:flex-row justify-between gap-2">
                        <div>
                          ...
                          <div className="break-all">@{winner.username}</div>
                          <div className="text-sm opacity-90">Number: {winner.winning_number}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-xl sm:text-2xl font-bold">{winner.prize_amount} ETB</div>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {activeTab === "leaderboard" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-4">📊 Winner Leaderboard</h2>
            <div className="space-y-2">
              {winnerLeaderboard.winners?.map((winner, idx) => (
                <div key={idx} className={`bg-gradient-to-r ${getPlaceBadge(winner.place)} rounded-xl p-3 text-white`}>
                  <div className="flex flex-col sm:flex-row justify-between gap-2">
                    <div>
                      <div className="font-bold break-all">@{winner.username}</div>
                      <div className="text-sm opacity-90">Number: {winner.winning_number} | Round {winner.game_id}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-lg sm:text-xl font-bold">{winner.prize_amount} ETB</div>
        ...
        {activeTab === "announcements" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-4">📢 Create Announcement</h2>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {announcements.slice(0, 10).map((ann, idx) => (
                <div key={idx} className="bg-gray-700 rounded-lg p-3">
                  <div className="text-white break-words text-sm">📢 {ann.text}</div>
